...
The project does not need any install step to run. It is meant to work by opening `main.html`.

The game uses TypeScript as the source file, but browsers run JavaScript, so `game.html` loads `scripts/game.js`.

If editing the TypeScript later, `scripts/game.js` should be updated too.
